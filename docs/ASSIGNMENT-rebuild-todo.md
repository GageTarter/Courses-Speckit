# Assignment: Create, Reset, and Rebuild the Courses Speckit App

**Goal:** Create your own GitHub repository from OC CS Speckit, strip the shipped courses implementation (answer key), then rebuild the application **from the feature specs** using Spec-Driven Development (SDD) and Cursor.
...
**Not this assignment:** Starting a **new** product from the starter kit — use [ASSIGNMENT-starter-kit.md](./ASSIGNMENT-starter-kit.md) instead. Reading-only tour of Speckit — use [ASSIGNMENT-walkthrough-courses.md](./ASSIGNMENT-walkthrough-courses.md).
...
Upstream (instructor / org repo): `https://github.com/OC-ComputerScience/courses.git`  
...
- **Repository name:** e.g. `courses-speckit` or `courses-<your-username>`
- **Visibility:** Public
...
3. Create the repository. Copy the HTTPS or SSH URL, e.g.
   `https://github.com/<you>/courses-speckit.git`

---

## Part B — Load the repo with courses-speckit

Start from a **ZIP download** so your local repo has a fresh git history (not a clone or fork of upstream).

1. Open the upstream repo: [OC-ComputerScience/courses](https://github.com/OC-ComputerScience/courses).
2. Click **Code → Download ZIP** (or use a course release ZIP if your instructor provides one).
3. Unzip into a folder named `courses-speckit` (or rename the unzipped folder).
...
5. Open a terminal in Cursor (Terminal → New Terminal) and enter the commands listed below. The terminal starts in the project folder, so you do not need to `cd`. Replace `<my-repo>` with the full GitHub URL you copied in Part A step 3 (e.g. `https://github.com/<you>/courses-speckit.git`):

```bash
git init
git add .
git commit -m "Initial import: OC CS Speckit courses example"
git branch -M main
...
```sql
CREATE DATABASE `courses-db`;
CREATE DATABASE `courses-db-test`;
```
...
`backend/.env` is for the running app; `backend/.env.test` is for automated tests (uses `courses-db-test` so tests do not wipe your dev data).  
...
| API      | [http://localhost:3200/courses/](http://localhost:3200/courses/) |
...
This is the core of the assignment: turn the answer key into a **scaffold + courses specs** baseline.
...
**What goes:** courses product implementation (models, API, UI, feature tests).  
**API prefix:** still `/courses/` so specs match.
...
4. Enter a commit message, e.g. `Reset courses example to scaffold; keep feature specs for rebuild`, then **Commit**.
...
"Reset courses example to scaffold; keep feature specs for rebuild"
...
git status
git add -A
git commit -m "Reset courses example to scaffold; keep feature specs for rebuild"
git push -u origin main
```
...
| 2     | [feature-2-courses-list-management.md](../features/feature-2-courses-list-management.md)           | `feature/2-courses-list-management`      |
| 3     | [feature-3-courses-list-item-management.md](../features/feature-3-courses-list-item-management.md) | `feature/3-courses-list-item-management` |
| 4     | [feature-4-user-profile-management.md](../features/feature-4-user-profile-management.md)     | `feature/4-user-profile-management`   |
| 5     | [feature-5-courses-due-date.md](../features/feature-5-courses-due-date.md)                         | `feature/5-courses-due-date`             |
...
2. Create / rename / delete courses lists on the dashboard.
3. Add / complete / edit / delete coursess; switch lists; deleting a list removes its coursess.
...
2. **Demo:** Demo your working system to the professor or GA (register/login, lists, coursess, and any other features required for your section).
...
2. `Reset courses example to scaffold; keep feature specs` on `main`
...
#   copy backend\.env.example backend\.env
#   copy backend\.env.test.example backend\.env.test
# CREATE DATABASE courses-db; CREATE DATABASE courses-db-test;

# Strip answer-key code
npm run reset:example -- --dry-run
npm run reset:example -- --yes
git add -A && git commit -m "Reset courses example to scaffold; keep feature specs"
...
| Confused with starter zip              | `starter:zip` = **new** product (no courses specs). This assignment uses `reset:example` |
